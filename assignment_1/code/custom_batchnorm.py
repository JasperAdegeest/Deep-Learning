...
class CustomBatchNormManualFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, gamma, beta, eps=1e-5):
        ...
        x_hat = input - mean
        inv_variance = 1 / torch.sqrt(variance + eps)
        normalized_input = x_hat * inv_variance
        out = normalized_input * gamma + beta

        ctx.save_for_backward(normalized_input, gamma, inv_variance)
    ...
    def backward(ctx, grad_output):
        ...
        normalized_input, gamma, inv_variance = ctx.saved_tensors

        B = grad_output.shape[0]
        dl_dx_hat = grad_output * gamma
        grad_input = B * dl_dx_hat - torch.sum(dl_dx_hat, dim=0)
        grad_input = grad_input - normalized_input * torch.sum(dl_dx_hat * normalized_input, dim=0)
        grad_input = grad_input * (1 / B) * inv_variance

        grad_gamma = torch.sum(grad_output * normalized_input, dim=0)
...
class CustomBatchNormManualModule(nn.Module):
    ...
    def forward(self, input):
        ...
        CustomBatchNorm = CustomBatchNormManualFunction()
        out = CustomBatchNorm.apply(input, self.gamma, self.beta, self.eps)
